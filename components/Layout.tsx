...
const Sidebar: React.FC<{ activePage: string, setActivePage: (page: string) => void }> = ({ activePage, setActivePage }) => {
  ...
  const adminNavItems = [
    ...
    { page: 'pre-management', label: 'PRE Management', icon: ICONS.users },
  ...
  const navItems = showAdminNav ? adminNavItems : preNavItems;
...
const Header: React.FC = () => {
    ...
    const isAdmin = user?.role === Role.ADMIN || user?.role === Role.DATACR;

    return (
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6">
            <h2 className="text-lg font-semibold text-slate-700">
                Welcome, {user?.name} {user?.role === Role.PRE && team ? <span className="text-sm font-normal text-slate-500 ml-2">({team})</span> : ''}
            </h2>
